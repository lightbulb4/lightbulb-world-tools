...
### Consolidate Mochie Packed Maps in Scene

Open **Tools > Lightbulb > Consolidate Mochie Packed Maps in Scene**, or **Find existing duplicate packed maps...** in the packer. Scan the active scene, review the keeper and replacement references for each group, and choose **Consolidate selected groups**.

- Scans editable scene materials with **Mochie/Standard** or **Mochie/Standard Lite**, including inactive objects, terrain templates and skyboxes. Checks active primary packed slots and Standard's active detail packed slots. Materials used only in other scenes, scripts, animations, or uninstantiated prefabs are excluded. Shared material assets still affect their other uses.
- Only Default / 2D PNG texture assets under `Assets` are compared. A SHA-256 shortlist is followed by a complete file-byte comparison. All serialized texture importer settings must match, including compression, mipmaps, filtering, wrap modes, size limits, and every platform override. Pending importer edits must be applied or reverted first. Files with equivalent pixels but different PNG encoding or metadata are deliberately not merged; there is no fuzzy matching.
- The texture with the first asset path in ordinal order is the stable keeper. Only duplicate texture references in the selected groups are replaced. Material assets remain distinct, and their AreaLit offsets, packed-map tiling, strengths, keywords, and other settings are unchanged. No Mochie source changes or packer invocation are needed for consolidation.
- Material state, active scene membership, texture contents, and import settings are revalidated before changes. The entire reference update supports **Edit > Undo**. Materials are not automatically saved; review and save afterward.
- No texture files are deleted or modified. Redundant files remain available for Undo and other uses. This consolidates the scene's references; it does not reclaim project disk space or guarantee a particular build-size reduction.
